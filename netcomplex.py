...
class ampBundle(layers.Layer):
  ...
  def call(self, Yre, Yim, Xre, Xim, Rre, Rim, Zre, Zim):
    ...
    Xtildere, Xtildeim = self.l1(Xre, Xim, Rre, Rim, self.M3re1, self.M3re2, self.M3im1, self.M3im2)
    ...
    Rre, Rim = self.l2(Yre, Yim, Xre, Xim, Rre, Rim, self.M3re1, self.M3re2, self.M3im1, self.M3im2)
...
class istaStage(layers.Layer):
  ...
  def __init__(self, p, *args):
    ...
    self.M2re1 = tf.Variable(initial_value=np.float32(M2.real),
                         trainable=True, name='M2re1')
    self.M2re2 = tf.Variable(initial_value=np.float32(M2.real),
                         trainable=True, name='M2re2')
    self.M2im1 = tf.Variable(initial_value=np.float32(M2.imag),
                         trainable=True, name='M2im1')
    self.M2im2 = tf.Variable(initial_value=np.float32(M2.imag),
  ...
  def call(self, Xre, Xim, Zre, Zim):
    ...
    Are = tf.matmul(self.M1re1, Zre) - tf.matmul(self.M1im1, Zim)
    Aim = tf.matmul(self.M1im2, Zre) + tf.matmul(self.M1re2, Zim)
    ...
    Bre = tf.matmul(Xre, tf.transpose(self.M2re1)) - tf.matmul(Xim, tf.transpose(self.M2im1))
    Bim = tf.matmul(Xre, tf.transpose(self.M2im2)) + tf.matmul(Xim, tf.transpose(self.M2re2))
    ...
    M2 = self.M2re1, self.M2re2, self.M2im1, self.M2im2
...
class ampLayer1(layers.Layer):
  ...
  def call(self, Xre, Xim, Rre, Rim, M3re1, M3re2, M3im1, M3im2):
    # import pdb; pdb.set_trace()
    # Xtilde = tf.transpose(tf.matmul(tf.transpose(R, perm=(0,2,1)), M3), perm=(0,2,1)) + X
    Xtildere = tf.matmul(M3re1, Rre) - tf.matmul(M3im1, Rim) + Xre
    Xtildeim = tf.matmul(M3im2, Rre) + tf.matmul(M3re2, Rim) + Xim
...
class ampLayer2(layers.Layer):
  ...
  def __init__(self, p, layerid, *args):
    ...
    self.M4re1 = tf.Variable(initial_value=np.float32(np.random.randn(p.M,p.M)/10),
                         trainable=True, name='M4re1')
    self.M4re2 = tf.Variable(initial_value=np.float32(np.random.randn(p.M,p.M)/10),
                         trainable=True, name='M4re2')
    self.M4im1 = tf.Variable(initial_value=np.float32(np.random.randn(p.M,p.M)/10),
                         trainable=True, name='M4im1')
    self.M4im2 = tf.Variable(initial_value=np.float32(np.random.randn(p.M,p.M)/10),
                         trainable=True, name='M4im2')
    
  def call(self, Yre, Yim, Xre, Xim, Rre, Rim, M3re1, M3re2, M3im1, M3im2):


    # Rnewre = Yre - (tf.matmul(tf.transpose(M3re1), Xre) - tf.matmul(-tf.transpose(M3im1), Xim))
    # Rnewim = Yim - (tf.matmul(tf.transpose(-M3im2), Xre) + tf.matmul(tf.transpose(M3re2), Xim))


    Rnewre = Yre \
      - (tf.matmul(tf.transpose(M3re1), Xre) - tf.matmul(-tf.transpose(M3im1), Xim)) \
      + tf.matmul(Rre, self.M4re1) - tf.matmul(Rim, self.M4im1)
        
    Rnewim = Yim \
      - (tf.matmul(tf.transpose(-M3im2), Xre) + tf.matmul(tf.transpose(M3re2), Xim)) \
      + tf.matmul(Rim, self.M4re2) - tf.matmul(Rre, self.M4im2)
